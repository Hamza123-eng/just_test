...
#define MAX_HTTP_RECV_BUFFER 512    //buffer for http
#define MAX_HTTP_OUTPUT_BUFFER 2048 //buffer for http
#define HASH_LEN 32
...
{
    return 0;
    //(ota_config->http_config->cert_pem || ota_config->http_config->use_global_ca_store || ota_config->http_config->crt_bundle_attach != NULL);
}
...
//     esp_http_client_set_header(https_ota_handle->http_client, "Authorization", hawkbit.hawkbit_authentication);
...
bool esp_ota(char *URL)
{
   // get_sha256_of_partitions();
    //ESP_LOGI(TAG, "Starting OTA ");
    ESP_LOGI(TAG, "DEPLOYEMNT URL:%s\n", URL);
